...
describe('Meal Tools', () => {
  ...
  describe('get_meal_events tool', () => {
    ...
    it('should handle service errors', async () => {
      ...
      const tool = mockServer.getTool('get_meal_events');
      // No try/catch in tool, so it throws
      await expect(tool?.execute({})).rejects.toThrow('API Error');
    });
  });

  describe('get_meal_event_details tool', () => {
    it('should be registered', () => {
      const tool = mockServer.getTool('get_meal_event_details');
      expect(tool).toBeDefined();
      expect(tool?.name).toBe('get_meal_event_details');
    });
    ...
    it('should retrieve specific meal event', async () => {
      ...
      expect(result.content[0].text).toContain('**Meal Event Details:**');
      expect(result.content[0].text).toContain('**Title:** Pasta Night');
      expect(result.content[0].text).toContain('**Date:** 2024-01-15');
      expect(result.content[0].text).toContain('**Recipe ID:** recipe-2');
      expect(result.content[0].text).toContain('**Recipe Scale Factor:** 1x');
    });
    ...
    it('should handle meal event without recipe', async () => {
      ...
      expect(result.content[0].text).toContain('**Title:** Sandwich Day');
      expect(result.content[0].text).toContain('**Date:** 2024-01-16');
      expect(result.content[0].text).toContain('**Recipe ID:** None');
    });
    ...
    it('should handle service errors', async () => {
      ...
      const tool = mockServer.getTool('get_meal_event_details');
      await expect(tool?.execute({ eventId: 'meal-1' })).rejects.toThrow('Retrieval failed');
    });
  });
  ...
  describe('create_meal_event tool', () => {
    ...
    it('should create meal event with recipe', async () => {
      ...
      // Actual format: name, date, and recipe ID inline
      expect(result.content[0].text).toContain('Successfully created meal event "Taco Tuesday"');
      expect(result.content[0].text).toContain('2024-01-20');
      expect(result.content[0].text).toContain('recipe ID: recipe-1');
    });
    ...
    it('should create meal event without recipe', async () => {
      ...
      expect(result.content[0].text).toContain('2024-01-21');
      // No recipe ID mentioned when none assigned
      expect(result.content[0].text).not.toContain('recipe ID');
    });
  ...
  describe('delete_meal_event tool', () => {
    ...
    it('should handle event not found', async () => {
      const tool = mockServer.getTool('delete_meal_event');
      // No try/catch, throws
      await expect(tool?.execute({ eventId: 'nonexistent' })).rejects.toThrow(
        'Meal event with ID nonexistent not found'
      );
    });
  });
  ...
  describe('get_weekly_meal_plan tool', () => {
    ...
    it('should get weekly meal plan', async () => {
      ...
      // Actual format: "Weekly meal plan (start to end):"
    ...
    it('should handle empty week', async () => {
      ...
      expect(result.content[0].text).toContain('Weekly meal plan');
      expect(result.content[0].text).toContain('No meals planned');
    });
  ...
  describe('Tool Registration', () => {
    it('should register all expected tools', () => {
      const expectedTools = [
        ...
        'get_meal_event_details',
        'create_meal_event',
        'update_meal_event',
        'delete_meal_event',
        'assign_recipe_to_meal',
        'get_weekly_meal_plan',
        'get_monthly_meal_plan',
        'get_meal_events_by_type',
        'bulk_create_meal_events',
